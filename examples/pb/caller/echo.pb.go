...
func NewEchoAPIClient(cc *qrpc.ClientConn) EchoAPIClient {
	cc.SetService("qrpc.test.api.EchoAPI.in")
	return &echoAPIClient{cc}
}

func (c *echoAPIClient) Echo(ctx context.Context, in *_pb.EchoRequest) (string, error) {
...
func _EchoAPI_Echo_Handler(srv interface{}, ctx context.Context, reqID string, msg []byte) error {
	out := new(_pb.EchoResponse)

	if err := proto.Unmarshal(msg, out); err != nil {
		return err
	}

	return srv.(EchoAPIServer).Echo(ctx, reqID, out)
}

var _EchoAPI_serviceDesc = qrpc.ServiceDesc{
	ServiceName: "qrpc.test.api.EchoAPI.out",
